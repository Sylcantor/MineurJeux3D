...
function createWalls(scene){
    ...
    // create the walls
    let wall1 = BABYLON.MeshBuilder.CreateBox("wall1", {width: 1000, height: 100, depth: 1}, scene);

    wall1.position = new BABYLON.Vector3(0, 0, 500);
    wall1.checkCollisions = true;
    wall1.physicsImpostor = new BABYLON.PhysicsImpostor(wall1, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 0 }, scene);

    let wall2 = BABYLON.MeshBuilder.CreateBox("wall2", {width: 1000, height: 100, depth: 1}, scene);
    wall2.position = new BABYLON.Vector3(0, 0, -500);
    wall2.checkCollisions = true;
    wall2.physicsImpostor = new BABYLON.PhysicsImpostor(wall2, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 0 }, scene);

    let wall3 = BABYLON.MeshBuilder.CreateBox("wall3", {width: 1, height: 100, depth: 1000}, scene);
    wall3.position = new BABYLON.Vector3(500, 0, 0);
    wall3.checkCollisions = true;
    wall3.physicsImpostor = new BABYLON.PhysicsImpostor(wall3, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 0 }, scene);

    let wall4 = BABYLON.MeshBuilder.CreateBox("wall4", {width: 1, height: 100, depth: 1000}, scene);
    wall4.position = new BABYLON.Vector3(-500, 0, 0);
    ...
    scene.meshes.push(wall1);
    scene.meshes.push(wall2);
    scene.meshes.push(wall3);
    scene.meshes.push(wall4);

    //add foor texture on walls
    let wallMaterial = new BABYLON.StandardMaterial("wallMaterial", scene);
   
    wallMaterial.diffuseTexture = new BABYLON.Texture("images/floor.jpg");

     //clone this material to avoid changing the original one
    let wallMaterial2 = wallMaterial.clone("wallMaterial");
    //decrease the texture repetition
    wallMaterial.diffuseTexture.uScale = 10.0;
    wallMaterial.diffuseTexture.vScale = 1.0;
    wallMaterial2.diffuseTexture.uScale = 1.0;
    wallMaterial2.diffuseTexture.vScale = 10.0;
    //assign the material to the walls
    wall1.material = wallMaterial;
    wall2.material = wallMaterial;
    wall3.material = wallMaterial2;
    wall4.material = wallMaterial2;
...
function createGround(scene) {
    ...
    function onGroundCreated() {
        ...
        groundMaterial.diffuseTexture = new BABYLON.Texture("images/floor.jpg");
...
function createTank(scene) {
    ...
    tank.fireLasers = function() { 
     ...
     //mettre une image sur le tank
        tank.material = new BABYLON.StandardMaterial("tank", scene);
        tank.material.diffuseTexture = new BABYLON.Texture("images/tank.jpg", scene)
...
function createHeroDude(scene) {
   // load the Dude 3D animated model
    ...
    BABYLON.SceneLoader.ImportMesh("him", "models/Dude/", "Dude.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
        ...
        heroDude.position = new BABYLON.Vector3(x - 10, 0, y);  // The original dude
        // make it smaller 
        //heroDude.speed = 0.1;
